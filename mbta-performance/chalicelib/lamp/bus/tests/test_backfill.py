...
class TestLoadSampleDays(unittest.TestCase):
    def setUp(self):
    ...
    def test_runs_the_pipeline_once_per_date_in_the_inclusive_range(self):
        with mock.patch.object(backfill, "generate_speed_segments") as generate:
            backfill.load_sample_days(date(2026, 9, 1), date(2026, 9, 3))
    ...
    def test_ensures_the_table_exists_before_writing(self):
        with mock.patch.object(backfill, "generate_speed_segments"):
            backfill.load_sample_days(date(2026, 9, 1), date(2026, 9, 1))

        self.ensure_table.assert_called_once_with("DeliveredTripMetricsBus", hash_key="route", range_key="date")

    def test_each_call_writes_to_dynamo(self):
        with mock.patch.object(backfill, "generate_speed_segments") as generate:
            backfill.load_sample_days(date(2026, 9, 1), date(2026, 9, 1))

        generate.assert_called_once_with(date(2026, 9, 1), upload=False, write_to_dynamo=True)

    def test_upload_flag_is_forwarded(self):
        with mock.patch.object(backfill, "generate_speed_segments") as generate:
            backfill.load_sample_days(date(2026, 9, 1), date(2026, 9, 1), upload=True)

        generate.assert_called_once_with(date(2026, 9, 1), upload=True, write_to_dynamo=True)
